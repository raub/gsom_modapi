...
static func build_corridor(
	...
	if direction == Vector2i.RIGHT or direction == Vector2i.LEFT:
		...
		var a_floor_y: float = a_room.center.y + door_bottom_for_side.call(a_coord, a_dir, a_room.height)
		var b_floor_y: float = b_room.center.y + door_bottom_for_side.call(b_coord, b_dir, b_room.height)
	...
	if direction == Vector2i.UP or direction == Vector2i.DOWN:
		...
		var a_floor_y_z: float = a_room.center.y + door_bottom_for_side.call(a_coord, a_dir_z, a_room.height)
		var b_floor_y_z: float = b_room.center.y + door_bottom_for_side.call(b_coord, b_dir_z, b_room.height)
...
static func __build_corridor_segment(
	...
	var lowest_floor: float = minf(start_floor_y, end_floor_y)
	var highest_floor: float = maxf(start_floor_y, end_floor_y)
	var start_point: Vector3 = (
		Vector3(center.x - length * 0.5, start_floor_y, center.z)
		if runs_x
		else Vector3(center.x, start_floor_y, center.z - length * 0.5)
	)
	var end_point: Vector3 = (
		Vector3(center.x + length * 0.5, end_floor_y, center.z)
		if runs_x
		else Vector3(center.x, end_floor_y, center.z + length * 0.5)
	)
	...
	var wall_height: float = corridor_height + (highest_floor - lowest_floor)
	...
	RoomLabsGeneratorBoxes.add_solid_box(
		...
		"corridor_ceiling",
		ceiling_size,
		Vector3(center.x, highest_floor + corridor_height + floor_thickness * 0.5, center.z),
	...
	var wall_size: Vector3 = (
		...
		if runs_x
		else Vector3(wall_thickness, wall_height, length)
	)
	...
	RoomLabsGeneratorBoxes.add_solid_box(
		...
		"corridor_wall_a",
		wall_size,
		Vector3(center.x, lowest_floor + wall_height * 0.5, center.z) + wall_offset_axis,
		RoomLabsGeneratorMaterials.mat_wall
	)
	RoomLabsGeneratorBoxes.add_solid_box(
		geom_root,
	...
	RoomLabsGeneratorLighting.add_ceiling_strip(
		geom_root,
		Vector3(center.x, lowest_floor, center.z),
		length * 0.8,
		corridor_width,
		room_height_default,
		runs_x,
